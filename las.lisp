...
(defun read-vlrs (header fd)
  (unless (zerop (number-of-variable-length-records header))
    (do ((vlr (read-value 'variable-length-record-header fd)
	      (read-value 'variable-length-record-header fd))
	 (i 0 (1+ i))
	 res)
...
(defun read-evlrs (header fd)
  (unless (zerop (number-of-evlrs header))
    (file-position fd (start-of-evlrs header))
    (do ((evlr (read-value 'extended-variable-length-record-header fd)
	       (read-value 'extended-variable-length-record-header fd))
	 (pos (file-position fd) (+ (file-position fd) (record-length-after-header evlr)))
	 (i 0 (1+ i))
	 res)
